...
///
/// <para>Only the four DXGI tags the build emits are accepted (R8G8B8A8 and BC7, each UNORM and
/// <c>_SRGB</c>); an unknown tag throws rather than guessing the <c>PITCH</c>/<c>LINEARSIZE</c> field.
/// Likewise a chain longer than the dimensions admit, or a level whose byte count isn't the format's
/// size, is refused — such a file parses as a DDS and then fails texture creation, surfacing as an
/// override that silently never binds. A TRUNCATED chain is legal (the flat maps ship one level).</para>
/// </summary>
internal static class DdsWriter
{
    ...
    public const uint R8G8B8A8_UNORM = 28, R8G8B8A8_UNORM_SRGB = 29, BC7_UNORM = 98, BC7_UNORM_SRGB = 99;
    ...
    public static void Write(Stream stream, uint dxgiFormat, int width, int height, IReadOnlyList<byte[]> levels)
    {
        ...
        if (!block && dxgiFormat is not (R8G8B8A8_UNORM or R8G8B8A8_UNORM_SRGB))
        ...
        var sizeFormat = block ? ATTextureFormat.BC7 : ATTextureFormat.RGBA32;
        int lw = width, lh = height;
        for (int i = 0; i < levels.Count; i++)
        {
            long want = TextureCodec.BlobSize(sizeFormat, lw, lh, 1);
            if (levels[i].Length != want)
                throw new ArgumentException(
                    $"mip level {i} is {levels[i].Length} bytes; {sizeFormat} at {lw}x{lh} is {want}", nameof(levels));
            lw = Math.Max(1, lw >> 1);
            lh = Math.Max(1, lh >> 1);
        }
        ...
        uint pitchOrLinearSize = block ? (uint)levels[0].Length : (uint)(width * 4);
