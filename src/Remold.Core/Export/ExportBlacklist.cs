...
/// (<see cref="Bundles.RosterBlacklist"/>), but the export entry points take a character and outfit
/// directly and so can be reached without it; they answer the same way — an empty result, silently and
/// with no user-visible trace. The listed character is a child NPC: don't remove entries, don't make the
