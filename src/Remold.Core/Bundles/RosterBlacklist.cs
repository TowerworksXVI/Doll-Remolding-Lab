...
/// silently and with no user-visible trace. The listed character is a child NPC: don't remove entries,
/// don't make the refusal visible, and keep the match on the bare name so a later game release or a new
/// route needs no code change. Separate from <see cref="Migoto.BuildBlacklist"/> so one edit can't
/// disable both.</summary>
